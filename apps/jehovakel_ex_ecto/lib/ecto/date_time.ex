...
defmodule Ecto.DateTime.Utils do
  ...
  defmacro is_time(hour, min, sec, usec \\ 0) do
    quote do
      unquote(hour) in 0..23 and unquote(min) in 0..59 and unquote(sec) in 0..59 and
...
defmodule Ecto.Date do
  ...
  defp do_cast(
         <<year::4-bytes, ?-, month::2-bytes, ?-, day::2-bytes, sep, _hour::2-bytes, ?:,
           _min::2-bytes, ?:, _sec::2-bytes, _rest::binary>>
       )
       when sep in [?\s, ?T],
       do: from_parts(to_i(year), to_i(month), to_i(day))
  ...
  def cast(%{"hour" => hour, "min" => min} = map),
    do:
      from_parts(
        to_i(hour),
        to_i(min),
        to_i(Map.get(map, "sec", 0)),
        to_i(Map.get(map, "usec", 0))
      )

  def cast(%{hour: hour, min: min} = map),
    do:
      from_parts(to_i(hour), to_i(min), to_i(Map.get(map, :sec, 0)), to_i(Map.get(map, :usec, 0)))
...
defmodule Ecto.DateTime do
  ...
  defp do_cast(
         <<year::4-bytes, ?-, month::2-bytes, ?-, day::2-bytes, sep, hour::2-bytes, ?:,
           min::2-bytes, ?:, sec::2-bytes, rest::binary>>
       )
       when sep in [?\s, ?T] do
    if usec = usec(rest) do
      from_parts(to_i(year), to_i(month), to_i(day), to_i(hour), to_i(min), to_i(sec), usec)
    else
      :error
    end
  end
  ...
  defp do_cast(
         %{"year" => year, "month" => month, "day" => day, "hour" => hour, "min" => min} = map
       ) do
    from_parts(
      to_i(year),
      to_i(month),
  ...
  defp do_cast(%{year: empty, month: empty, day: empty, hour: empty, minute: empty})
       when empty in ["", nil] do
    {:ok, nil}
  end

  defp do_cast(
         %{"year" => year, "month" => month, "day" => day, "hour" => hour, "minute" => min} = map
       ) do
    from_parts(
      to_i(year),
      to_i(month),
  ...
  defp do_cast({{year, month, day}, {hour, min, sec}}) do
    from_parts(to_i(year), to_i(month), to_i(day), to_i(hour), to_i(min), to_i(sec), 0)
  end

  defp do_cast({{year, month, day}, {hour, min, sec, usec}}) do
    from_parts(to_i(year), to_i(month), to_i(day), to_i(hour), to_i(min), to_i(sec), to_i(usec))
  end
  ...
  def from_date(%Ecto.Date{year: year, month: month, day: day}) do
    %Ecto.DateTime{year: year, month: month, day: day, hour: 0, min: 0, sec: 0, usec: 0}
  end
  ...
  def to_iso8601(%Ecto.DateTime{
    ...
    str =
      zero_pad(year, 4) <>
        ...
        "-" <>
        zero_pad(day, 2) <>
        "T" <> zero_pad(hour, 2) <> ":" <> zero_pad(min, 2) <> ":" <> zero_pad(sec, 2)
